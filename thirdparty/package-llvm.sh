...
# 2.  Delete the mlir and test folders to keep the archive small.
# 3.  Unpack the IWYU tarball in tools/clang/tools/include-what-you-use
# 4.  Create new tarball from the resulting source tree
#
# Usage:
...
if [[ "${1:-}" == "-f" ]]; then
    rm -rf llvm-project-$VERSION.src llvm-$VERSION.src include-what-you-use \
      llvm-$VERSION.src \
