...
MultiRaftHeartbeatBatcherPtr MultiRaftManager::AddOrGetBatcher(
  ...
  if (res && (batcher = res->lock())) {
    return batcher;
  }
  auto raft_pool_token = raft_pool_->NewToken(ThreadPool::ExecutionMode::CONCURRENT);
  batcher = std::make_shared<MultiRaftHeartbeatBatcher>(
      hostport, dns_resolver_, messenger_, batch_time_window_, move(raft_pool_token));
  batcher->StartTimer();
...
void MultiRaftManager::Shutdown() {
  std::unordered_map<HostPort, std::weak_ptr<MultiRaftHeartbeatBatcher>, HostPortHasher> batchers;
  {
    std::lock_guard lock(mutex_);
