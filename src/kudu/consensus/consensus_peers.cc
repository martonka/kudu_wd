...
void Peer::SendNextRequest(bool even_if_queue_empty, MultiRaftConsensusData* mrc_data) {
  ...
  if (req_has_ops) {
    ...
    if (multi_raft_batcher_) {
      ...
      // DCHECK(mrc_data == nullptr) << "Messages with ops should not be sent on MRC thread";
      // Should be always true. But could cause a deadlock if we are on the MRC thread.
