...
TEST_F(CodegenTest, CodegenRandomSchemas) {
  ...
  FLAGS_codegen_cache_capacity = kNumThreads * kNumProjectionsPerSchema / 5;

  // Make sure there is enough space the the codegen compilation queue to
  ...
  for (size_t thread_idx = 0; thread_idx < kNumThreads; ++thread_idx) {
    threads.emplace_back([&, thread_idx = thread_idx]() {
      ...
      while (!stop) {
        // Choose a random number: this is the number of columns in the schema.
        ...
        // from the 'column schema library' in a random manner.
        ...
        for (auto idx : idx_seq) {
          if (!seen_idx.emplace(idx).second) {
            // A duplicate index, ignore and continue.
            continue;
          }
        ...
        for (size_t iter = 0; iter < kNumProjectionsPerSchema; ++iter) {
          Schema projection;
          {
            vector<size_t> col_indices(gen() % seen_idx.size());
            std::iota(col_indices.begin(), col_indices.end(), 0);
            std::shuffle(col_indices.begin(), col_indices.end(), gen);

            VLOG(3) << StringPrintf("thread %2zd: %2zd-column projection",
                                    thread_idx, col_indices.size());
        ...
        // Drop some of the references to keep at most
        // kNumProjectionsPerSchema / 2 between the cycles.
        size_t count = 0;
